...
Let $U$ and $V$ be independent $\text{Uniform}(a=0, b=1)$ random variables. 
What is the p.d.f. of $T = U + V$?
  
  We have to convolve the p.d.f. of $U$ with the p.d.f. of $V$. The p.d.f.s of both $U$ and $V$ are 
\[ f(x) = \begin{cases} 1 & 0 < x < 1 \\ 0 & \text{otherwise} \end{cases}. \]
This is a rectangle function.

To calculate the convolution, we first flip $f_V$ about the $y$-axis. We then integrate the 
product of $f_U$ and the flipped version of $f_V$ to get the convolution at $t=0$. In this case, 
...
```{r, echo=FALSE, engine='tikz', out.width='50%', fig.ext='png', fig.align='center', engine.opts = list(template = "tikz_template.tex")}
  ...
  \draw[gray,->] (-1.5, -2) -- (2.5, -2);
	...
	\node[anchor=north] at (1, -2.1) {$1$};
		...
		\draw[ultra thick,blue,opacity=0.5] (-1.4, 0) -- (0, 0) -- (0, 1) -- (1, 1) -- (1, 0) -- (2.4, 0);
		\node[anchor=west,blue] at (1, 1) {$f_U$};
	
	\draw[ultra thick,red,opacity=0.5] (-1.4, 0) -- (-1, 0) -- (-1, 1) -- (0, 1) -- (0, 0) -- (2.4, 0);
	\node[anchor=east,red] at (-1, 1) {$f_V$};
...
```
  ...
  Now, to get the convolution at $t$, we shift the flipped version of $f_V$ to the right by $t$. This is why 
  the convolution operation is often called "flip and shift". To see why, consider the formula when we 
  flip and shift $f_V$:
  \begin{align*}
  \text{FLIP}[f_V](x) &= f_V(-x) \\
  \text{SHIFT}_t[\text{FLIP}[f_V]](x) &= f_V(-(x - t)) = f_V(t - x). \\
  \end{align*}
  This is exactly the term that we multiply by $f_U(x)$ in the convolution formula \@ref(eq:convolution-continuous) 
  before integrating.
  
  For the uniform distribution, there are two cases. If we shift (the flipped version of $f_V$) by some amount 
  $0 < t < 1$, then the supports of the two distributions overlap between $0$ and $t$, so the result of the convolution 
  is
...
```{r, echo=FALSE, engine='tikz', out.width='50%', fig.ext='png', fig.align='center', engine.opts = list(template = "tikz_template.tex")}
  ...
  \draw[gray,->] (-1.5, -2) -- (2.5, -2);
	...
	\draw[ultra thick,red,opacity=0.5] (-1.4, 0) -- (-0.6, 0) -- (-0.6, 1) -- (0.4, 1) -- (0.4, 0) -- (2.4, 0);
	\node[anchor=east,red] at (-0.6, 1) {$f_V$};
...
\draw[gray,->] (-1.5, -2) -- (2.5, -2);
	...
	\draw[ultra thick,red,opacity=0.5] (-1.4, 0) -- (0.5, 0) -- (0.5, 1) -- (1.5, 1) -- (1.5, 0) -- (2.4, 0);
	\node[anchor=west,red] at (1.5, 1) {$f_V$};
...
exponential random variables, we can use \@ref{thm:convolution-continuous} to derive its p.d.f. 
