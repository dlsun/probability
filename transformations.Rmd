...
This is a special case of a more general problem. We have a random variable $X$ 
whose distribution we know, and we want to find the distribution of $Y = g(X)$, 
a transformation of $X$. The strategy for these problems is this:

1. Find the c.d.f. of $Y$. 
2. Take the derivative to obtain the p.d.f.
...
&= \begin{cases} 0 & x < 0 \\ P(L \leq 4 \sqrt{x}) & x \geq 0 \end{cases} \\
&= \begin{cases} 0 & x < 0 \\ \sqrt{x} & 0 \leq x \leq 1 \\ 1 & x > 1 \end{cases}.
\end{align*}

Now we take the derivative to get the p.d.f.
\[ f_A(x) = F'_A(x) = \begin{cases} \frac{1}{2\sqrt{x}} & 0 \leq x \leq 1 \\ 0 & \text{otherwise} \end{cases}. \]
We graph the p.d.f.s of $L$ and $A$ below.
...
\[ f_Y(y) = F'_Y(y) = \frac{1}{a} f_X(\frac{y}{a}). \]
