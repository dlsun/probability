...
1. Calculate the p.m.f. of $Y = g(X)$, then calculcate $E[Y]$ from the usual formula \@ref(eq:ev).
2. Use the **Law of the Unconscious Statistician** (LOTUS), described below.

```{theorem lotus, name="LOTUS"}
Let $X$ be a random variable with c.d.f. $f_X(x)$. Define $Y = g(X)$ for some function $g$. Then, 
$E[Y] = E[g(X)]$ is 
\begin{equation}
...
The result is called the "Law of the Unconscious Statistician" because it is so 
intuitive that people apply the theorem without even thinking about it. 
Remember that $E[g(X)]$ represents the "average" value of $g(X)$. To calculate the average value 
of $g(X)$, it is intuitive that we take a weighted average of the possible values $g(x)$, where the
weights are the probabilities $f_X(x)$.

Let's first do an example where the Law of the 

Now let's look at the problem in the lesson.
...
Here is an example of LOTUS applied to a p.m.f. in formula form. This particular expected value 
may seem somewhat arbitrary, but it will come in handy later.

```{example binomial-lotus, name="Expected Value of a Binomial Random Variable"}
...
\[ f(x) = \binom{n}{x} \frac{N_1^x N_0^{n-x}}{N^n}, x=0, 1, \ldots, n. \]
  ...
  Next, we replace $x(x-1) \cdot \binom{n}{x}$.
  \begin{align*}
...
**moment generating function** of $X$, while engineers may recognize this function as the 
**Laplace transform** of the p.m.f. of $X$.


## Additional Exercises {-}

1. Let $X$ be a $\text{Hypergeometric}(n, N_1, N_0)$ random variable. Calculate $E[X(X-1)]$.

2. Let $X$ be a $\text{Poisson}(\mu)$ random variable for $0 < \mu < 1$. Calculate $E[X!]$.
