...
random variable.
  ...
  \end{align*}
  where the only thing we changed in going from the first line to the second was to start the sum at $x=1$ 
    instead of $x=0$. (We can do this because the summand is 0 when $x=0$.) 
  Now we use replace $x \cdot \binom{n}{x}$ using the identity \@ref(eq:committee-chair):
  \begin{align*}
