...
It is clear that we would need to win more than $1 for each 
dollar we bet, to make it worthwhile to play this game. But what would a 
fair amount be?
  
## Theory {-}

So far in this class, we have described random variables by their p.m.f. or 
...
Now let's answer the question posed in the lesson. Clearly, the casino's payout of 35-to-1 is still unfavorable to us. 
But what would the payout need to be to make the game fair? In other words, the p.m.f. is 
...
random variable:
  ...
  &= \sum_{x=1}^n x \cdot \binom{n}{x} \frac{N_1^x N_0^{n-x}}{N^n} & \text{(start sum at $x=1$, since first term is $0$)} \\
  &= \sum_{x=1}^n x \cdot \frac{n!}{x! (n-x)!} \frac{N_1^x N_0^{n-x}}{N^n} & \text{(expand definition of binomial coefficient)} \\
  &= \sum_{x=1}^n \frac{n!}{(x-1)! (n-x)!} \frac{N_1^x N_0^{n-x}}{N^n} & \text{(cancel $x$ with $x! = x \cdot (x-1)!$)} \\
  &= \sum_{x'=0}^{n-1} \frac{n!}{x'! (n-1-x')!} \frac{N_1^{x' + 1} N_0^{n - 1 - x'}}{N^n} & (x' = x - 1) \\
  &= n \frac{N_1}{N} \sum_{x'=0}^{n-1} \underbrace{\frac{(n-1)!}{x'! (n-1-x')!}}_{\binom{n-1}{x'}} \frac{N_1^{x'} N_0^{n - 1 - x'}}{N^{n-1}} & (\text{pull factors of $n$, $N_1$, $N$ outside the sum}) \\
  &= n \frac{N_1}{N} 
  \end{align*}
  
In the last equality, we used the fact that the expression inside the sum is the p.m.f. is a 
$\text{Binomial}(n-1, N_1, N_0)$ random variable. Since we are summing over all the possible values of this 
random variable ($0, 1, \ldots n-1$), the sum must equal 1.
```

Now, if we know that a random variable $X$ has a binomial distribution, we can use the formula 
\[ E[X] = n\frac{N_1}{N} \]
instead of calculating it from scratch using \@label(eq:ev) and the p.m.f. We can 
...
Assume packet arrivals meet the assumptions of a Poisson process. How many arrivals would you expect to see 
