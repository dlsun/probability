...
geometric, except one that stops after $3$ $\fbox{1}$s have been drawn 
...
we need the $r$th $\fbox{1}$ to come on the $x$th draw. Otherwise, we have 
...
Then, we have:
\begin{align*}
f(x) &= \binom{x-1}{r-1} \frac{N_0^{x-r} \cdot N_1^r}{N^x} \\
...
$\fbox{1}$s are drawn sooner, so the $r=5$th $\fbox{1}$ happens earlier.
