...
The definition of the variance for continuous random variables is exactly the 
same as the definition for discrete random variables, except the expected 
...
The stanard deviation is also defined in the same way, as the square root of the variance, 
...
Let $X$ be a $\text{Uniform}(a, b)$ random variable. 
\begin{align*}
\text{Var}[X] &= E[X^2] - E[X]^2 \\
...
Let $X$ be a $\text{Exponential}(\lambda)$ random variable. 
\begin{align*}
\text{Var}[X] &= E[X^2] - E[X]^2 \\
...
rate of 6 per hour. 
    ...
    a. What is the expected value and standard deviation of the time between two arrivals?
