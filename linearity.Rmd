...
Here is a particularly clever application of linearity.

```{example}
Let $X$ be a $\text{Binomial}(n, N_1, N_0)$ random variable. What is $E[X(X-1)]$? 
In Lesson \@ref(lotus), we calculated this expected value using LOTUS. Here is a
...
But $E[Y_{ij}]$ is simplyl the probability that tickets $i$ and $j$ are both $\fbox{1}$s. 
This probability is $E[Y_{ij}] = \frac{N_1^2}{N^2}$. Therefore, 
...
in the year on which at least two of these people were born. (_Hint:_ Express this complicated random variable as 
a sum of $0/1$ random variables, and use linearity of expectation.)
