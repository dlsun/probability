...
\begin{align*}
P(\text{red on 5th spin}\ |\ \text{not red on first 4 spins}) &= \frac{P(\text{not red on first 4 spins and red on 5th spin}) }{P(\text{not red on first 4 spins})} \\
...
\begin{align*}
P(A \text{ and } B) &= P(A) P(B | A) & \text{by the Multiplication Rule} \\
&= P(A) P(B) & \text{by assumption}.
\end{align*}

Conversely, we assume \@ref(eq:independence-mult) and show that \@ref(eq:independence) holds:
\begin{align*}
P(B | A) &= \frac{P(A \text{ and } B)}{P(A)} & \text{by the definition of conditional probability}  \\
...
&= 1 - P(\text{1st die is 1, 2, 3, 4 and 2nd die is 1, 2, 3, 4}) \\
