...
To visualize the variance function, we take its square root so that it is in the same units as the 
mean function. Then, we plot the band $\mu(t) \pm \sqrt{V(t)}$ around the mean function $\mu(t)$:
...
The variance function of the Poisson process is visualized below as a band of width 
$\sqrt{V(t)} = \sqrt{\lambda t}$ around the mean function $\mu(t) = \lambda t$, which 
we calculated in Example \@ref(exm:poisson-process-mean).
...
variance function of Brownian motion.
