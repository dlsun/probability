...
In Lesson \@ref(lti-time), we looked at what LTI filters do to stationary signals in the time 
domain. However, it is usually much easier to analyze and understand LTI filters in the frequency 
domain. 
...
Frequencies below 0.6 Hz are passed (by a factor of $1.5^2 = 2.25$), while 
...
We can then integrate the PSD $S_Y(f)$ of the output to obtain the expected power 
in this frequency band. But by the Filter Theorem (Theorem \@ref(thm:filter-theorem)), we have 
\[ \int_{-\infty}^\infty S_Y(f)\,df = \int_{-\infty}^\infty |H(f)|^2 S_X(f)\,df = \int_{a < |f| < b} S_X(f)\,df, \]
since $|H(f)|^2$ is 0 outside $a < |f| < b$.
```
...
```{r, echo=FALSE, engine='tikz', out.width='70%', fig.ext='png', fig.align='center', engine.opts = list(template = "tikz_template.tex")}
	...
	\node (x) at (0, 0) {$X(t)$};
	\node[draw,blue] (h) at (2, 0) {$H$};
	\node (y) at (4, 0) {$Y(t)$};

  \draw[->,gray] (1.0, 0.5) -- (3.0, 0.5);
  \draw[->,gray] (2, 0.4) -- (2, 1.2);
  \node[anchor=west] at (3.0, 0.5) {{\small $f$}};
...
3. Let $\{ X(t) \}$ be a random process with mean function 
