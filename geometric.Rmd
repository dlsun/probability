...
_until_ a $\fbox{1}$s is drawn, then its p.m.f. is given by 
\begin{equation}
...
the random variable has a $\text{Geometric}(p)$ distribution, where $p$ is the parameter.
```

Note that in contrast to the hypergeometric and binomial distributions, there is no upper 
bound on the possible values of the geometric distribution. We could keep drawing $\fbox{0}$s, so 
in theory, we might need to wait an arbitrarily long time to get a $\fbox{1}$.
...
```{proof}
There are $N^x$ ordered ways to choose $x$ tickets from $N$ with replacement. Recall from Lesson 
...
In order for the first $\fbox{1}$ to appear on the $x$th draw, the first 
...
(\#eq:geometric-count)
\end{equation}
since we are drawing with replacement (so the composition of the box stays the same between draws).
...
not surprisingly, lower values of the geometric random variable become more probable.
