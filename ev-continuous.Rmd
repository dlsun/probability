...
```{example, name="Expected Value of the Uniform Distribution"}
...
```{r ev-continuous, echo=FALSE, engine='tikz', out.width='70%', fig.ext='png', fig.align='center', fig.cap='Expected Value of the Uniform Distribution', engine.opts = list(template = "tikz_template.tex")}
	\begin{tikzpicture}
		...
		\draw [<->] (-2, 0) coordinate -- (2, 0) coordinate;
    ...
    \node[anchor=north] at (1, 0) {$b$};
			...
			\draw[ultra thick,blue] (-2, 0) coordinate -- (-1, 0) coordinate -- (-1, 1) coordinate -- (1, 1) coordinate -- (1, 0) coordinate -- (2, 0) coordinate;
	
	\end{tikzpicture}
```

```{example, name="Expected Value and Median of the Exponential Distribution"}
